...
class Torch_QR_EVT_v1(drl.Agent):
    ...
    def __init__(self, env, **kwargs):
        ...
        self.lower_quantiles = 0.05#0.14#0.14#0.05#0.2#0.09#0.1#0.3 # set alpha for CVaR calculation
        self.thresh_quantile = 0.95#0.9#0.95#0.75#0.75  # the quantile level after which we assume lower confidence
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            #
            tau_quart_target_batch = (Variable(torch.rand(self.expectation_num, 1)).to(self.device))*(self.thresh_quantile) # 0 - \eta
            ...
            #
            thresh_next_state_action_values = self.target_critic(next_st_batch_thresh, next_action_batch.float(), self.kr1, mode=False)
            ...
            '''lower_expected_state_action_batch = reward_batch.repeat(self.expectation_num, 1) \
                + (self.gamma * done_batch*(lower_next_state_action_values + expon_samples*scaling_factor))
    ...
    def CVaR(self, state_batch, kr1):
        ...
        cvar = self.critic_model(rep_thresh_state_batch, action_batch, self.kr1, mode=False) 
    ...
    def save(self, env, trial):
        ...
        actor_path = "{}/{}_Torch_QR_EVT-v1_actor_{}".format(results_dir, env, trial)
        critic_path = "{}/{}_Torch_QR_EVT-v1_critic_{}".format(results_dir, env, trial)
        scale_path = "{}/{}_Torch_QR_EVT-v1_scale_{}".format(results_dir, env, trial)
