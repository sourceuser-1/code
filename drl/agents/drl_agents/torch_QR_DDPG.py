...
class Torch_QR_DDPG(drl.Agent):
    ...
    def __init__(self, env, **kwargs):
        ...
        self.batch_size = 128#64#128#64
        self.hidden_size = 128#128#400#128

        self.expectation_num = 100 # number of samples to take expectation over
        self.kr1 = 0.0 # dropout for ist layer of MLP
        self.lower_quantiles = 0.01#0.05#0.1#0.1 # set alpha for CVaR calculation
        ...
        self.gamma = 0.99#kwargs['gamma']#0.99
        # Setup Optimizers
    ...
    def update(
        ...
        next_state_action_values = self.target_critic(next_st_batch, next_action_batch.float(), self.kr1, mode=False)

        done_batch =done_batch.repeat(self.expectation_num, 1)

        # RHS of Bellman Equation
        expected_state_action_batch = reward_batch.repeat(self.expectation_num, 1) + (self.gamma * done_batch*next_state_action_values)
    ...
    def action(self, state, add_noise):
        ...
        noise = 0.2*np.random.randn(sampled_actions.shape[0], sampled_actions.shape[1])
    ...
    def save(self, env, trial):
        ...
        actor_path = "{}/{}_TorchQR_DDPG-v0_actor_{}".format(results_dir, env, trial)
        critic_path = "{}/{}_TorchQR_DDPG-v0_critic_{}".format(results_dir, env, trial)
