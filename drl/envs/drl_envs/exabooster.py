...
class ExaBooster_v2(gym.Env):
    def __init__(self):
        ...
        booster_model_file = 'fullbooster_noshift_e25_bs32_k_invar6_outvar2_axis1_mmscaler_t0_D03012021-T173721_kfold4__final.h5'
        booster_dir = os.path.dirname(__file__)
        print('booster_dir',booster_dir)
        ...
        self.booster_model.summary()
        # try:
        #     booster_data_dir = self.save_dir
        # except:
        #     sys.exit("Must set booster_data_dir")
        # booster_data_dir = 'None'
        # if booster_data_dir == 'None':
        ...
        # Continuous
        self.action_space = spaces.Box(low=np.percentile(data['B:VIMIN_DIFF'], 5),
                                       high=np.percentile(data['B:VIMIN_DIFF'], 95),
                                       shape=(1,),
                                       dtype=np.float32)
    ...
    def step(self, action):
        ...
        #if self.steps % 10 == 0:  # so over this rendering...
        #    self.render()
        reward_list = [np.asscalar(reward), np.asscalar(rach_reward), np.asscalar(data_reward)]
        #print(self.state[0, :, -1:].shape)
        #
        self.rl_reward_list.append(float(reward))
        avg_rl_reward = np.mean(self.rl_reward_list[-20:])
        self.avg_rl_reward_list.append(avg_rl_reward)
        #
        self.rachael_reward_list.append(float(rach_reward))
        avg_rachael_reward = np.mean(self.rachael_reward_list[-20:])
        self.avg_rachael_reward_list.append(avg_rachael_reward)
        #
        self.data_reward_list.append(float(data_reward))
        avg_data_reward = np.mean(self.data_reward_list[-20:])
        self.avg_data_reward_list.append(avg_data_reward)

        if self.steps % 25 == 0:
            fig, axs = plt.subplots(1, figsize=(12, 12))
            plt.plot(self.avg_data_reward_list, label='data reward')
            plt.plot(self.avg_rachael_reward_list, label='PID Eq reward')
            plt.plot(self.avg_rl_reward_list, label='RL agent reward')
            plt.legend()
            plt.savefig('results_dir/reward_episode{}_step{}.png'.format(self.episodes, self.steps))
            plt.close('all')
